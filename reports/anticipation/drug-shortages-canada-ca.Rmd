...
a) Using data from [Drug Shortages Canada](https://www.drugshortagescanada.ca/) and [Drug Product Database (DPD)](https://www.canada.ca/en/health-canada/services/drugs-health-products/drug-products/drug-product-database/what-data-extract-drug-product-database.html), first identify the number of actual drug shortages reported to Health Canada between June 01 2019 and June 01 2021.

b) Next, identify the proportion of drugs found in a) that did not have a prior report of an anticipated or actual shortage within a 12-month period for the same drug. 

c) For those drugs that had the type of report that was of concern in b), what was the average age on the market?
...
I'm going to filter through the data I've pulled from the website and keep only the records that have an `actual_start_date`. I'm assuming this means the shortage actually occured.

```{r filter-data-1}
actual_shortage <- drug_shortages_data_within_range %>%
  filter(!is.na(actual_start_date))
```
...
### Finding the Unanticipated Shortages

Next I'm going to find the unanticipated shortages by filtering the data from the first part, keeping only the records that failed to include an anticipated start date for the shortage.

```{r filter-data-2}
did_not_anticipate_at_all <- actual_shortage %>%
  filter(is.na(anticipated_start_date)) %>%
  filter(!is.na(actual_start_date))
```
...
actual_shortage %>%
  group_by(en_drug_brand_name) %>%
  summarise()
  
prior_report <- function(table) {
  
  #' table containing reports with the same din.
  #' find the
  
  #' step 1: get the drugs that had an actual shortage
  #' step 2: get check if they have anticipated shortage reports in the past year,
  #' step 3: group by drug name
  #' step 4: check each drug, see if it had a prior report
...
According to the code above, there were `r nrow(did_not_anticipate_at_all)` unanticipated shortages.

</div>

***

### Finding Inaccurately Reported Shortages

These are shortages that were not anticipated within a 12 month time-frame.

```{r filter-data-3}
did_not_anticipate_within_12_mo <- actual_shortage %>%
  filter(!is.na(anticipated_start_date)) %>%
  filter(!is.na(actual_start_date)) %>%
  filter(abs(actual_start_date - anticipated_start_date) > 365)
```

<div class = "blue">

According to the code above, there were `r nrow(did_not_anticipate_within_12_mo)` inaccurately anticipated shortages.
...
drugs_of_interest <- rbind(did_not_anticipate_within_12_mo, did_not_anticipate_at_all)
...
According to the code above, we can say there were `r nrow(drug_shortages_data_within_range)` shortage reports between the dates of `r my_start_date` and `r my_end_date`. Of those reports, there were `r nrow(actual_shortage)` actual shortages.

Of the actual shortages, there were `r nrow(did_not_anticipate_at_all)` unanticipated shortages. This means the drug did not have any previous shortage anticipation whatsoever. Of the actual shortages, there were `r nrow(did_not_anticipate_within_12_mo)` inaccurately anticipated shortages. This means the drug did have a shortage report in the past but it was reported more than 12 months before an actual shortage.
...
The proportion for the above is `r (nrow(did_not_anticipate_within_12_mo) + nrow(did_not_anticipate_at_all)) / (actual_shortage %>% nrow())`.
