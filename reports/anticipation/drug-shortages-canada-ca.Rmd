...
b) Next, identify the proportion of the reports found in a) that did not have a prior report of an anticipated or actual shortage within a 12-month period for the same drug. 
...
my_date_range <- interval(start = my_start_date, end = my_end_date, tzone = "America/Toronto")
...
```{r select-columns}
drug_shortages_data %<>%
  select(din, created_date, anticipated_start_date, estimated_end_date, actual_start_date, actual_end_date, decision_reversal, updated_date, status, en_comments, en_drug_brand_name, en_drug_common_name, resolved, avoided, advance, hc_updated_date, en_supply_disruption_mitigation, en_discontinuation_comments, hc_en_comments, drug.current_status, drug.company.created_date, type.id, type.label)
```
...
```{r age-on-market}
my_drug_product_database_status_marketed <- actual_shortage %>%
  filter(drug.current_status == "MARKETED") %>%
  mutate(HISTORY_DATE = as.Date(drug.company.created_date)) %>%
...
did_not_anticipate_within_12_mo$din %<>%
  as.character(did_not_anticipate_within_12_mo$din) %>%
  str_pad(8, pad = "0")

my_drug_product_database_status <- merge(my_drug_product_database_all_files_status,
                                         did_not_anticipate_within_12_mo,
...
- There were `r nrow(actual_shortage)` actual shortages.
- There were `r nrow(did_not_anticipate_at_all)` unanticipated shortages.
- There were `r nrow(did_not_anticipate_within_12_mo)` inaccurately anticipated shortages.
- The proportion for the above is `r did_not_anticipate_within_12_mo %>% nrow() / actual_shortage %>% nrow()`.
